...
def generate_recruiter_report_pdf(job_id: str) -> dict:
    ...
    top_candidate_name = "No candidates"
    top_candidate_score = "-"
    if top:
        top_candidate = top.get('candidate')
        if isinstance(top_candidate, dict):
            top_candidate_name = top_candidate.get('name', 'Unknown')
        else:
            top_candidate_name = str(top_candidate or 'Unknown')
        top_candidate_score = _fmt_pct(top.get('score', 0))

    summary_lines = [
        f"<b>Total Applicants:</b> {total}",
        f"<b>Average Score (All):</b> {_fmt_pct(avg_score)}",
        f"<b>Top Candidate:</b> {top_candidate_name} ({top_candidate_score})"
    ]
